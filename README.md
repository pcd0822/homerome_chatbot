...
바꾸면 됩니다. Claude 는 **기본(Sonnet)** 과 **고급(Opus)** 두 슬롯으로 나뉘어 있어,
학생이 화면 하단 칩에서 "Claude"(기본)와 "Claude 고급"(복잡한 작업용)을 골라 씁니다.
두 슬롯은 같은 키(`ANTHROPIC_API_KEY`)를 공유합니다. 더 저렴하게 가려면 기본을
`claude-haiku-4-5` 로 낮추세요.

```ts
export const MODELS = {
  // Claude 기본(Sonnet): 빠르고 저렴 — 대부분의 질문은 이걸로 충분
  anthropic:      { model: 'claude-sonnet-5', envKey: 'ANTHROPIC_API_KEY', maxTokens: 16000 },
  // Claude 고급(Opus): 복잡한 추론·긴 문서용 — 비용 높음, 필요한 학생만 선택
...
기본 선택은 **Claude 기본(Sonnet 5)** 이고, 학생이 필요할 때 "Claude 고급(Opus)" 을
직접 고를 수 있습니다. `maxTokens` 는 답변 길이 상한이며 학급 규모·비용에 맞게 조정하세요.
